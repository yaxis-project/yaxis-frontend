...
const useMetaVault = () => {
  const { account, ethereum }: { ethereum: provider, account: any } = useWallet();
  ...
  const calcMinTokenAmount = async (amounts: string[]) => {
    ...
    try {
      ...
      if (contract) {
        ...
        const tokensDeposit = await contract.methods.calc_token_amount_deposit(params).call()
        const threeCrvDeposit = amounts[3] || '0'
        return new BigNumber(tokensDeposit).plus(threeCrvDeposit)
  ...
  const onWithdraw = useCallback(
    async (sharesAmount: string, currencyAddress: string) => {
      setSubmitting(true)
      const params = [new BigNumber(sharesAmount).toFixed(0), currencyAddress]
      try {
  ...
  const onGetRewards = useCallback(
    async () => {
      setClaiming(true)
      try {
