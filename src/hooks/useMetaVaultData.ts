...
import {collapseDecimals, getYaxisMetaVault, getYaxisMetaVaultConverter, numberToDecimal} from '../yaxis/utils';
...
function useMetaVaultData(id: string): IHookReturn {
	...
	const {slippage, pickleWithdrawFee, vaultWithdrawFee, strategy} = useMetaVault()
	...
	const [metaVaultData, setMetaVaultData] = useState<MetaVaultData>({} as MetaVaultData)

	const vaultConfig = currentConfig.vault;
	const tokenAddresses: string[] = useMemo(() => {
		return [vaultConfig.dai,
			vaultConfig.usdc,
			vaultConfig.usdt,
	...
	const [allowances, onUpdateAllowances] = useAllowances(tokenAddresses,
	...
	const [daiWithdrawal, usdcWithdrawal, usdtWithdrawal, threeCrvWithdrawal] = estimatedWithdrawals;
	...
	useEffect(() => {
		(async () => {
			try {
				...
				const {balance, name, image, availableAmount, currenciesData} = nomalizeData(rawData);
				setBalance(balance);
				setName(name);
				setImage(image);
	...
	const fetchMetaVaultData = useCallback(async () => {
		try {
			...
			if (contract) {
				const [balance, userInfo, totalStaked, pendingYax, totalSupply, yaxPerBlock,rewardMultiplier]: any[] = await Promise.all([
				...
				const rewardPerBlock = new BigNumber(collapseDecimals(new BigNumber(yaxPerBlock).multipliedBy(rewardMultiplier)));
				data.initialized = true;
				data.stake = collapseDecimals(userInfo.amount)
				data.balance = collapseDecimals(balance)
				data.totalBalance = collapseDecimals(new BigNumber(balance).plus(userInfo.amount))
        ...
        data.strategy = strategy;
				data.tvl = new BigNumber(data.totalStaked).multipliedBy(cure3CrvPrice).toNumber()
				data.apy = getApy(Number(data.tvl),
					yaxPrice, data.rewardPerBlock, 1);
				data.mvltPrice = new BigNumber(totalStaked).div(totalSupply).multipliedBy(cure3CrvPrice).toNumber()
	...
	const callEstimateWithdrawals = useCallback(async (balanceShares) => {
		setEstimating(true)
		try {
			...
			if (contract) {
				...
				const values = await Promise.all(tokenAddresses.map(async addr => {
					if (new BigNumber(balanceShares).gt(metaVaultData.totalSupply)) {
						return '0'
					}
					const total3CrvInPool = numberToDecimal(metaVaultData.totalStaked || 0, 18)
					const r = new BigNumber(total3CrvInPool).multipliedBy(shares).dividedBy(metaVaultData.totalSupply).toFixed(0);
					const withdrawnFee = new BigNumber(1).times(1 - vaultWithdrawFee).times(1 - pickleWithdrawFee);
					let isCrv = addr == currentConfig.vault.threeCrv;
					const value = isCrv ? r : await contract.methods.calc_token_amount_withdraw(r, addr).call();
					// console.log('DEBUG_LOG==>>: withdrawnFee', {withdrawnFee: withdrawnFee.toString()});
					let real = new BigNumber(value)
						.times(withdrawnFee).times(isCrv ? 1 : (1 - slippage))
						.toFixed();
					// console.log('DEBUG_LOG==>>: r,shares,value,addr,isCrv,real', {r: r.toString(),shares: shares.toString(),value: value.toString(),addr,isCrv,real: real.toString()});
					return real
				}))
		...
		setEstimating(false)
	}, [yaxis, slippage, setEstimatedWithdrawals, tokenAddresses, metaVaultData.totalBalance])

	useEffect(() => {
		if (account && ethereum && yaxis && yaxis.web3 && yaxPrice && cure3CrvPrice && block) {

			fetchMetaVaultData()
		}
	}, [account, ethereum, yaxis, fetchMetaVaultData, yaxPrice, cure3CrvPrice, block])
