...
/* TODO: combine into useContractWrite */
...
const useApprove = (contract: Contract, address: string, token?: string) => {
	...
	const approve = useMemo(
		() =>
			initialized &&
			contract?.methods?.approve(address, ethers.constants.MaxUint256),
	...
	const handleApprove = useCallback(async () => {
		try {
			...
			if (!approve)
				throw new Error('Internal Error. Please reset and try again.')
