 ...
 */
export async function getYAXPriceData(selectedDay: SelectableDay, setYaxData: Function) {
  ...
  try {
    ...
    setYaxData({values: values, dates: dates, max: max(values) * 1.2 , min: min(values) * 0.8});
  } catch {
