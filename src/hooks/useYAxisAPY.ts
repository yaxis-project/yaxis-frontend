    ...
    async (_priceMap) => {
      try {
        ...
        let decodedWeights = aggCallsWeights[1].map((hex: any, i: number) => {
          ...
          return [
            weightCalls[i][0],
            (yaxis.web3.eth.abi.decodeParameter('uint256', hex) as any) / 1e18,
          ]
        })
        ...
        let workingSupplies = decodedRate
          ...
          .map((v: number) => v / 1e18)
        let virtualPriceCalls = Object.values(poolInfo).map((v: any) => [
          v.swap,
          '0xbb7b8b80',
        ])
        ...
        let decodedVirtualPrices = aggVirtualPrices[1].map(
          (hex: any, i: number) => {
            ...
            return [
              ...
              (yaxis.web3.eth.abi.decodeParameter('uint256', hex) as any) /
                1e18,
            ]
          },
        )
        ...
        decodedWeights.forEach((w: any, i: number) => {
          ...
          let virtual_price = decodedVirtualPrices.find(
            (v: any) => v[0].toLowerCase() === swap_address.toLowerCase(),
          )[1]
