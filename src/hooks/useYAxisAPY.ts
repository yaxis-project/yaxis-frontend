...
const useYAxisAPY = () => {
  ...
  const getYAxisAPY = useCallback(async (_priceMap) => {
    try {
      console.log("GET YAXIS APY")
      ...
      const gaugeController_address = '0x2F50D538606Fa9EDD2B11E2446BEb18C9D5846bB'
      const gauge_relative_weight = '0x6207d866000000000000000000000000'
      const { BTC: btcPrice, CRV: CRVprice } = _priceMap
      const weightCalls = decodedGauges.map(gauge => [gaugeController_address, gauge_relative_weight + gauge.slice(2)])
      let aggCallsWeights = await multicallContract.methods.aggregate(weightCalls).call()
      let decodedWeights = aggCallsWeights[1].map((hex: any, i: number) => {
        // @ts-ignore
        return [weightCalls[i][0], yaxis.web3.eth.abi.decodeParameter('uint256', hex) / 1e18]
      })
      let ratesCalls = decodedGauges.map(gauge => [
        [gauge, "0x180692d0"],
        [gauge, "0x17e28089"],
      ])
      let aggRates = await multicallContract.methods.aggregate(ratesCalls.flat()).call()
      let decodedRate = aggRates[1].map((hex: any) => {
        // @ts-ignore
        return yaxis.web3.eth.abi.decodeParameter('uint256', hex)
      })
      let gaugeRates = decodedRate.filter((_: any, i: number) => i % 2 === 0).map((v: number) => v / 1e18)
      let workingSupplies = decodedRate.filter((_: any, i: number) => i % 2 === 1).map((v: number) => v / 1e18)
      let virtualPriceCalls = Object.values(poolInfo).map((v: any) => [v.swap, "0xbb7b8b80"])
      let aggVirtualPrices = await multicallContract.methods.aggregate(virtualPriceCalls).call()
      let decodedVirtualPrices = aggVirtualPrices[1].map((hex: any, i: number) => {
        // @ts-ignore
        return [virtualPriceCalls[i][0], yaxis.web3.eth.abi.decodeParameter('uint256', hex) / 1e18]
      })
      const apys: any = {}
      decodedWeights.forEach((w: any, i: number) => {
        // @ts-ignore
        let pool: string = Object.values(poolInfo).find((v: any) => v.gauge.toLowerCase() === '0x' + weightCalls[i][1].slice(34).toLowerCase()).name
        let swap_address = poolInfo[pool].swap
        let virtual_price = decodedVirtualPrices.find((v: any) => v[0].toLowerCase() === swap_address.toLowerCase())[1]
        ...
        if (isNaN(apy))
          apy = 0
        // @ts-ignore
        Object.values(poolInfo).find((v: any) => v.name === pool).gauge_relative_weight = w[1]
        console.log(pool, apy, "POOL YAXIS APY")
        apys[pool] = apy
      })
      setYAxisAPY(apys)
