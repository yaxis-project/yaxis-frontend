...
export default function useYaxisStaking(currency: Currency) {
  ...
  const getData = async () => {
    try{
      const rate = await yaxis.contracts.xYaxStaking.methods.getPricePerFullShare().call();
      const data = {
        ...balances,
        walletBalance,
        ...
        stakedBalanceUSD: new BigNumber(rate).div(1e18).multipliedBy(priceMap?.YAX).times(sBalance).div(1e18),
      };
      setBalances(data);
    } catch (err) {
    }
