...
const Balances: React.FC = () => {
	...
	const [stakingSupply, setStakingSupply] = useState<BigNumber>(new BigNumber(0))
	...
	const stakingTvl = useMemo((): number => {
		return new BigNumber(stakingSupply).div(1e18)
	...
	if (stakedValues && stakedValues.length) {
		const t = farms.reduce((c, {active}, i) => c + (active ? (stakedValues[i].tvl || 0) : 0), 0)
		tvl = tvl.plus(t)
	}
							...
							<StyledBalances>
								<StyledBalance>
									...
									<div style={{flex: 1}}>
										...
										<Value value={!!account ? tvl.toNumber() : 'Locked'}/>
									</div>
								</StyledBalance>
							</StyledBalances>
						</CardContent>
				...
				<Col xs={24} md={12}>
					<Card>
						<CardContent>
							...
							<Value
								value={
									yaxisPrice ? new BigNumber(yaxisPrice).toFixed(2) : 'Locked'
								}
							/>
						</CardContent>
					</Card>
							...
							<StyledBalances>
								<StyledBalance>
									...
									<div style={{flex: 1}}>
										...
										<Value
											value={
												!!account ? getBalanceNumber(yaxisBalance) : 'Locked'
											}
										/>
									</div>
								</StyledBalance>
						...
						<Footnote>
							...
							<FootnoteValue>
								<Value
									...
									inline={true}
									value={
										!!totalAmount ? getBalanceNumber(totalAmount) : 'Locked'
									}
								/> YAX
							</FootnoteValue>
						</Footnote>
