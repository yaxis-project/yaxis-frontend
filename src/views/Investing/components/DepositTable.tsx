 ...
 * @param value
 */
const handleFormInputChange = (setCurrencyValues: Function) => (key: string, value: string) => {
...
export default function DepositTable() {
  ...
  const [currencyValues, setCurrencyValues] = useState<CurrencyValues>(initialCurrencyValues);

  const { currenciesData, onUpdateAllowances } = useMetaVaultData('v1');

  const disabled = useMemo(() => computeInsufficientBalance(currencyValues, currenciesData), [currencyValues]);
  ...
  const currenciesNeededApproval = useMemo((): ICurrency[] =>
    pipe(
      mapObjIndexed((v: string, symbol: string) => {
        const currency = currenciesData.find(c => c.tokenId === symbol);
        if (!currency) return [];
        const value = (new BigNumber(v || 0)).times(new BigNumber(10).pow(currency.decimals));
        if (value && !value.isNaN() && currency && value.gt(currency.allowance) && value.lte(currency.balance)) {
          return [currency];
        }
        return [];
      }),
  ...
  return (<div className="deposit-table">
