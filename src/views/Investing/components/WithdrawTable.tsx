...
const WithdrawalSelector = (props: WithdrawalSelectorProps) => {
  const { availableCurrencies, withdrawValueUSD, withdrawValueShares, withdrawDisabled } = props;
  const languages = useContext(LanguageContext);
  const language = languages.state.selected;
  const t = (s: string) => (phrases[s][language]);
  const { onAddTransaction } = useTransactionAdder();
  const { onWithdraw } = useMetaVault()
  const [withdrawalCurrency, setWithdrawCurrency] = useState<Currency | undefined>(Dai);
  ...
  const handleSubmit = async () => {
    ...
    const selectedCurrency = find(currenciesData, c => c.tokenId === withdrawalCurrency.tokenId);
    try {
      const receipt = await onWithdraw(sharesAmount, selectedCurrency.address);
      onAddTransaction({
  ...
  return <Row className='to-wallet'>
    <Col span={8}>
    ...
    <Col span={14}>
      <Select 
        ...
        onSelect={
            selected => setWithdrawCurrency(
              find(availableCurrencies, curr => (curr.name === selected))
            )
          }
        >
        { availableCurrencies.map(currency => (<Option 
            value={currency.name}
            key={currency.name}
          >
            <img
      ...
      <Text className='title' type="secondary">{t("You'll receive an estimate of")}</Text>
      <Value 
      ...
      />
      <br/>
      <Button className="investing-btn"
...
const useWithdrawValueHandler = () => {
  ...
  const { metaVaultData: { totalBalance, mvltPrice } } = useMetaVaultData('v1');
  ...
  const withdrawValueShares = useMemo(() => (withdrawValueUSD.div(mvltPrice)), [withdrawValueUSD, mvltPrice]);

  return { withdrawValueUSD, setWithdrawValueUSD, withdrawValueShares, totalAvailableInUSD, totalBalance };
}
...
export default function WithdrawTable() {
  ...
  const { withdrawValueUSD, setWithdrawValueUSD, withdrawValueShares, totalAvailableInUSD, totalBalance } = useWithdrawValueHandler();

  const updateWithdraw = (value: string) => setWithdrawValueUSD(value == "" ? (new BigNumber(0)) : (new BigNumber(value)));

  const withdrawalError = withdrawValueUSD.gt(totalAvailableInUSD);
  const withdrawDisabled = withdrawValueUSD.isLessThanOrEqualTo(new BigNumber(0)) || withdrawalError;

  return (<div className="withdraw-table">
      ...
      <Col span={14}>
        <Text className='title'>
          ...
          <Text type="secondary" className='available'>
            ...
            <Tooltip
              title={<>
                { (new BigNumber(totalBalance || 0)).toFixed(2) + " MVLT" }
              </>}
            >
              {" $" + totalAvailableInUSD.toFixed(2)} <img src={info} style={{ position: 'relative', top: -2 }} height="15" alt="Withdraw Token Breakdown" />
            </Tooltip>
          </Text>
        <Form.Item
          validateStatus={withdrawalError && 'error'}
        >
          <Input
            placeholder="0"
            type="number"
