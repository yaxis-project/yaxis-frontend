...
export default function DepositAssetRow(props: DepositAssetRowProps) {
  ...
  const currencyData = find(currenciesData, curr => curr['name'] === currency.name);
  const balance = currencyData ? new BigNumber(currencyData.balance) : new BigNumber('0');
  ...
  return (<>
    <Row className='deposit-asset-row'>
      ...
      <Col span={7} className='balance'>
        <Value value={balance.toNumber()} numberPrefix='$' decimals={2} />
        <Text type="secondary">{value} {currency.name}</Text> 
      </Col>
        ...
        >
          <Input
            ...
            suffix={<>
              ...
              &nbsp;
              <Button block size='small' onClick={maxAmount}>MAX</Button>
            </>}
