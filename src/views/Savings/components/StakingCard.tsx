...
const TableHeader = (props: any) => (<Col span={props.span}>
  <Text style={{ float: props.float || 'none' }} type="secondary">{props.value}</Text>
</Col>)
...
const StakingRow = (props: StakingRowProps) => {
  ...
  return (
    <>
      <Row gutter={gutter}>
        <Col span={6}>
            <Value value={getBalanceNumber(walletBalance)} decimals={2} numberSuffix=" YAX" />
        </Col>
        <Col span={12}>
            <Value value={getBalanceNumber(stakedBalance)} decimals={2} numberSuffix=" YAX" />
        </Col>
      </Row>
    </>
  )
...
export default function StakingCard() {
  ...
  const { stakedBalance, walletBalance, rate, sYaxBalance } = useYaxisStaking(YAX);
  ...
  const approveYAX = async () => {
    try {
      ...
      notification.info({
        message: t("An error occurred during YAX staking approval. Please try again.")
      });
    }
  };
  ...
  const unstakeYAX = async () => {
    try {
      ...
      notification.info({
        message: t("An error occurred during YAX unstaking. Please try again.")
      });
    }
  ...
  const updateDeposit = (value: string) => setDeposit(value == "" ? "0" : value);
  const errorDeposit = useMemo(() => (new BigNumber(depositAmount)).gt(walletBalance.div(1e18)), [walletBalance, depositAmount]);
  const depositDisabled = useMemo(() => ((new BigNumber(depositAmount)).eq(new BigNumber(0)) || errorDeposit), [depositAmount]);

  const [withdrawAmount, setWithdraw] = useState<string>("0");
  const updateWithdraw = (value: string) => setWithdraw(value == "" ? "0" : value);
  const errorWithdraw = useMemo(() => (new BigNumber(withdrawAmount)).gt(stakedBalance), [stakedBalance, withdrawAmount]);
  const withdrawDisabled = useMemo(() => ((new BigNumber(withdrawAmount)).eq(new BigNumber(0)) || errorWithdraw), [withdrawAmount]);
  ...
  return (
    <Card className="staking-card" title={<strong>{t("Staking")}</strong>}>
      ...
      <Row gutter={24}>
        ...
        <Col span={12} className={'balance'}>
          <img
            ...
            alt="logo"
          />
          <Value value={stakedBalance.toNumber()} decimals={2} numberSuffix=" YAX" />
        </Col>
      </Row>
      ...
      <Row gutter={24}>
        <Col span={12}>
          <Form.Item
            validateStatus={errorDeposit && 'error'}
          >
            <Input
          ...
          {
            !allowance.toNumber() ? 
            <Button className="staking-btn" onClick={approveYAX} block type="primary">{t("Approve")}</Button> :
            <Button className="staking-btn" disabled={depositDisabled} onClick={stakeYAX} block type="primary">{t("Deposit")}</Button>
          }
        </Col>
        <Col span={12}>
          <Form.Item
            validateStatus={errorWithdraw && 'error'}
          >
            <Input
          ...
          <Button className="staking-btn" disabled={withdrawDisabled} onClick={unstakeYAX} block type="primary">{phrases["Withdraw"][language]}</Button>
        </Col>
      </Row>
    </Card>
  )
