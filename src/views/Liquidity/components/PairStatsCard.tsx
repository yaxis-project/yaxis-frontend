...
export default function LiquidityOverviewCard() {
  ...
  const [stakedValue, setStakedValue] = useState<StakedValue>(defaultStakedValue);
  const { farmData: { pid } } = useLPContractData(
    'YAX',
    UNI_ETH_YAX_LP
  );
  ...
  return <DetailOverviewCard title="Pair Stats">
    <DetailOverviewCardRow>
      <Text>Total Value Locked</Text>
      <Value value={new BigNumber(stakedValue.tvl).toNumber()} numberPrefix="$" decimals={2} />
    </DetailOverviewCardRow>
    <DetailOverviewCardRow>
    ...
    <DetailOverviewCardRow>
      ...
      {props?.lpTokens[0] &&
        <Value value={stakedValue?.reserve[0]} decimals={0} numberSuffix={` ${props?.lpTokens[0].symbol}`} />}
      {props?.lpTokens[1] &&
        <Value value={stakedValue?.reserve[1]} decimals={0} numberSuffix={` ${props?.lpTokens[1].symbol}`} />}
    </DetailOverviewCardRow>
  </DetailOverviewCard>
}