...
import {
  Row, Col, Typography, Card, Button, Divider, Input, Radio
} from 'antd';
    ...
    <>
      <Row gutter={gutter} style={{ paddingTop: 25, paddingBottom: 25 }}>
        <Col span={6}>
          <img
            src={strategy.icon}
            height="32" style={{ marginTop: -4, }}
            alt="logo"
          />
          <Text>{strategy.name}</Text>
        </Col>
        <Col span={6}>
          <Text strong style={{ marginRight: 10 }}>+ </Text>
          <img
            src={YAX.icon}
            height="32" style={{ marginTop: -4,  }}
            alt="logo"
        ...
        <Col span={6}>
          <Row>
            <CheckOutlined style={{ marginLeft: 40, marginTop: 5 }} />
          </Row>
        </Col>
        <Col span={6}>
          <Row>
...
export default function () {
  ...
  const language = languages.state.selected;
  const gutter = 5;
  return (<Card bodyStyle={{ padding: 12 }}>
    <Row gutter={gutter}>
      <TableHeader value={phrases["Strategy"][language]} span={12} />
      <TableHeader value={phrases["Current Strategy"][language]} span={6} />
      <TableHeader value={phrases["Select Vote"][language]} span={6} />
    </Row>
    <Divider plain style={{ margin: 0 }} />
    ...
    <Row gutter={gutter} style={{ marginTop: 10 }}>
      <Col offset={16} span={8}>
        <Button style={voteButtonStyle} block type="primary">{phrases["Confirm Vote"][language]}</Button>
      </Col>
    </Row>
  </Card>)
}