...
function fetchClaim(account: string, chainId: number): Promise<UserClaimData | null> {
    ...
    return (CLAIM_PROMISES[key] =
        ...
        new Promise((resolve) => {
            ...
            .then((res: any) => (res.ok ? res.json() : console.log(`No claim for account ${account} on chain ID ${chainId}`)))
...
export function useUserHasAvailableClaim(account: string | null | undefined): boolean {
    ...
    const isClaimedResult = useSingleCallResult(contracts?.internal?.merkleDistributor, 'isClaimed', [userClaimData?.index])
...
export function useUserUnclaimedAmount(account: string | null | undefined): false | UseUserUnclaimedAmountResponse {
    const userClaimData = useUserClaimData(account)
