...
export const getAllowances = async (yaxis: Yaxis, mutilcall: Contract, tokens: string[], owner: string, spender: string): Promise<BigNumber[]> => {
	const calls: any[] = [];
	...
	try {
		...
		return response[1].map((value: any) => {
			// @ts-ignore
			let decodeUint256 = yaxis.web3.eth.abi.decodeParameter('uint256', value) as string;
...
export const getBalances = async (yaxis: Yaxis, mutilcall: Contract, tokens: string[], owner: string): Promise<BigNumber[]> => {
	const calls: any[] = [];
	const testToken = getContract(yaxis.web3.currentProvider)
	tokens.forEach(token => {
		calls.push([
			token,
			testToken.methods.balanceOf(owner).encodeABI()
		]);
	});
	try {
		const response = await mutilcall.methods.aggregate(calls).call();
