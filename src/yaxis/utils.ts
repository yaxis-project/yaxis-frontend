...
export async function getCoinGeckoPrices() {
	...
	let prices = await (await fetch(`${COINGECKO_API}?vs_currencies=usd&ids=${cgkIds}`)).json()
	const priceMap: any = {}
...
export function collapseDecimals(value: any, decimal = 18) {
	return value ? new BigNumber(value).div(new BigNumber(10).pow(decimal)).toString() : "0"
}
...
export const getYaxisPrice = (stakedValue: any, farms: any[]) => {
	const yaxisIndex = farms.findIndex(({tokenSymbol}) => tokenSymbol === 'YAX')
...
export const getFarms = (yaxis: Yaxis): Farm[] => {
	return yaxis
		? yaxis.contracts.pools.map(pool => <Farm>{
				...pool,
				id: pool.symbol,
				lpToken: pool.symbol,
...
export const getPoolWeight = async (yaxisChefContract: Contract, pid: number) => {
...
export const getTotalStaking = async (yaxis: Yaxis) => {
	return new BigNumber(await yaxis.contracts.xYaxStaking.methods.availableBalance().call())
}
export const getXSushiStakingContract = (yaxis: Yaxis) => {
	return yaxis && yaxis.contracts && yaxis.contracts.xYaxStaking
}

export const getEarned = async (yaxisChefContract: Contract, pid: number, account: string) => {
	return yaxisChefContract.methods.pendingYaxis(pid, account).call()
}

async function getUniPoolInfo(yaxisChefContract: Contract, priceMap: any, farm: Farm): Promise<StakedValue> {
	...
	const reserve = [numberToFloat(_reserve0, reserveTokens[0].decimals),
		numberToFloat(_reserve1, reserveTokens[1].decimals)]
	let totalSupply = numberToFloat(await lpContract.methods.totalSupply().call());

	const balance = numberToFloat(await lpContract.methods
		.balanceOf(yaxisChefContract.options.address)
		.call())
	const prices = [priceMap[reserveTokens[0].symbol], priceMap[reserveTokens[1].symbol]]
	if (prices[1]) {
...
async function getBalPoolInfo(yaxisChefContract: Contract, priceMap: any, farm: Farm): Promise<StakedValue> {
	...
	const _reserve0 = await lpContract.methods.getBalance(reserveTokens[0].address).call();
	const _reserve1 = await lpContract.methods.getBalance(reserveTokens[1].address).call();
	const reserve = [numberToFloat(_reserve0, reserveTokens[0].decimals),
		numberToFloat(_reserve1, reserveTokens[1].decimals)]
	...
	if (prices[1]) {
		prices[0] = (prices[1] * reserve[1] * weights[0]) / (reserve[0] * weights[1])
	} else if (prices[0]) {
		prices[1] = (prices[0] * reserve[0] * weights[1]) / (reserve[1] * weights[0])
	}
	const totalLpValue = reserve[0] * prices[0] + reserve[1] * prices[1]
	const lpPrice = new BigNumber(totalLpValue).div(totalSupply).toNumber()
	const tvl = balance * lpPrice;
...
export const approve = async (lpContract: Contract, yaxisChefContract: Contract, account: string) => {
	return lpContract.methods
		.approve(yaxisChefContract.options.address, ethers.constants.MaxUint256)
		.send({from: account})
}

export const callApprove = async (erc20Contract: Contract, spender: string, account: string) => {
...
export const getYaxisSupply = async (yaxis: Yaxis) => {
	return new BigNumber(await yaxis.contracts.yaxis.methods.totalSupply().call())
}

export const stake = async (yaxisChefContract: Contract, pid: number, amount: string, account: string) => {
...
export const unstake = async (yaxisChefContract: Contract, pid: number, amount: string, account: string) => {
	return yaxisChefContract.methods
		.withdraw(
			pid,
			new BigNumber(amount).times(new BigNumber(10).pow(18)).toString(),
...
export const harvest = async (yaxisChefContract: Contract, pid: number, account: string) => {
	return yaxisChefContract.methods
		.deposit(pid, '0')
		.send({from: account})
		.on('transactionHash', (tx: any) => {
...
export const depositAll = async (yAxisMetaVault: Contract, params: any[], account: string) => {
	return yAxisMetaVault.methods
		.depositAll
		.apply(null, params)
		.send({from: account})
		.on('transactionHash', (tx: any) => {
...
export const withdraw = async (yAxisMetaVault: Contract, params: any[], account: string) => {
	return yAxisMetaVault.methods
		.withdraw
		.apply(null, params)
		.send({from: account})
...
export const getStaked = async (yaxisChefContract: Contract, pid: number, account: string) => {
	try {
 ...
 * @param account 
 */
export const enter = async (contract: Contract, amount: string, account: string) => {
...
export const leave = async (contract: Contract, amount: string, account: string) => {
	return contract.methods
		.leave(
...
export const getCurveApyApi = async () => {
	try {
		const {apy = {}} = await (await fetch('https://www.curve.fi/raw-stats/apys.json')).json()
		return apy?.total && apy?.total['3pool'] ? parseFloat(apy?.total['3pool']) : 0
	} catch (e) {
		return 0
	}
}
export const getPickle3CrvAPY = async (contract: Contract, pickleJarContract: Contract, picklePrice: number, _3crvPrice: number, block: number) => {
	const [poolInfo, totalAllocPoint, picklePerBlock, multiplier, ratio, balance]: any[] = await Promise.all([
	...
	const tvl = numberToFloat(new BigNumber(balance).times(ratio).toNumber(), 18, 18) * _3crvPrice
	const rewardPerBlock = Number(collapseDecimals(new BigNumber(picklePerBlock).times(multiplier), 18))
	let poolWeight = new BigNumber(poolInfo.allocPoint).dividedBy(totalAllocPoint).toNumber();
	let farmApy = getApy(tvl,
		picklePrice, rewardPerBlock, poolWeight);
	return farmApy
}
