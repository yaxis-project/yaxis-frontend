...
export async function supply() {
	let totalSupply = numberToFloat(
		await yaxContract.methods.totalSupply().call(),
		18,
	)
	let yaxAmountInChef = numberToFloat(
		await yaxContract.methods.balanceOf(yaxChefAddress).call(),
		18,
	)
	let yaxAmountInUniLp = numberToFloat(
		await yaxContract.methods.balanceOf(yaxEthLpAddress).call(),
		18,
	)
